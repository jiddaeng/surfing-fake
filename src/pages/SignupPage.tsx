...
export function SignupPage() {
  ...
  const [form, setForm] = useState({ name: '', studentNumber: '', email: '', password: '', confirmPassword: '' })
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!/^[^\s@]+@dimigo\.hs\.kr$/i.test(form.email.trim())) return setError('학교 이메일(@dimigo.hs.kr)을 입력해주세요.')
    ...
    <div className="min-h-screen bg-gradient-to-br from-brand-50 via-white to-blue-50 px-4 py-6 sm:py-10">
      <div className="mx-auto max-w-lg">
        ...
        <div className="card mt-8 rounded-[1.75rem] p-6 shadow-xl shadow-brand-100/60 sm:p-8">
          ...
          <h1 className="mt-5 text-2xl font-black tracking-tight">DIMI CLUB 시작하기</h1>
          ...
          <form onSubmit={submit} className="mt-7 space-y-4">
            ...
            <label className="block"><span className="mb-2 block text-sm font-semibold">학교 이메일</span><div className="relative"><Mail size={17} className="absolute left-3.5 top-3.5 text-gray-400" /><input type="email" maxLength={320} pattern="[^@\\s]+@dimigo\\.hs\\.kr" value={form.email} onChange={(e) => update('email', e.target.value)} className={`${inputClass} pl-10`} placeholder="student@dimigo.hs.kr" autoComplete="username" required /></div></label>
