    ...
    <div className="mx-auto max-w-6xl">
      ...
      <section className="card mt-6 p-5 sm:p-7">
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2">
          ...
          {club && <div className="sm:col-span-2"><Field label="동아리 로고" hint="PNG, JPG 또는 WEBP · 최대 2MB"><label className="focus-ring flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-gray-300 bg-gray-50 px-4 py-6 text-sm font-semibold text-gray-600 hover:border-brand-300 hover:bg-brand-50"><ImagePlus size={18} /> {uploading ? '업로드 중...' : '로고 이미지 선택'}<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={(e) => logoUpload(e.target.files?.[0])} /></label></Field></div>}
        </div>
      </section>
      ...
      <section className="card mt-5 overflow-hidden">
        ...
        <div className="grid min-h-[28rem] lg:grid-cols-2">
          <div className={`${tab === 'preview' ? 'hidden lg:block' : ''} border-gray-100 lg:border-r`}><textarea maxLength={20000} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="h-full min-h-[28rem] w-full resize-none border-0 bg-gray-950 p-5 font-mono text-sm leading-7 text-gray-100 outline-none sm:p-7" spellCheck={false} /></div>
          <div className={`${tab === 'edit' ? 'hidden lg:block' : ''} overflow-auto p-5 sm:p-7`}><div className="markdown"><SafeMarkdown>{form.description || '*소개를 작성하면 여기에 미리보기가 표시됩니다.*'}</SafeMarkdown></div></div>
        </div>
      </section>
      ...
      <section className="mt-8">
        ...
        <div className="mt-4 space-y-3">{draftQuestions.map((question, index) => <div key={question.id} className="card p-4 sm:p-5"><div className="flex gap-3"><GripVertical size={19} className="mt-3 shrink-0 text-gray-300" /><div className="min-w-0 flex-1"><div className="grid gap-3 sm:grid-cols-[1fr_10rem]"><input maxLength={500} value={question.label} onChange={(e) => patchQuestion(question.id, { label: e.target.value })} className={inputClass} placeholder={`질문 ${index + 1} 내용을 입력하세요`} /><select value={question.type} onChange={(e) => patchQuestion(question.id, { type: e.target.value as QuestionType, options: ['single','multiple'].includes(e.target.value) ? question.options ?? ['선택지 1', '선택지 2'] : undefined })} className={inputClass}>{Object.entries(QUESTION_TYPE_LABEL).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div><input maxLength={2000} value={question.description ?? ''} onChange={(e) => patchQuestion(question.id, { description: e.target.value })} className={`${inputClass} mt-3`} placeholder="질문에 대한 추가 설명 (선택)" />
                  {['single', 'multiple'].includes(question.type) && <div className="mt-3 space-y-2">{(question.options ?? []).map((option, optionIndex) => <div key={optionIndex} className="flex gap-2"><input maxLength={200} value={option} onChange={(e) => patchQuestion(question.id, { options: question.options?.map((item, i) => i === optionIndex ? e.target.value : item) })} className={inputClass} placeholder={`선택지 ${optionIndex + 1}`} /><button onClick={() => patchQuestion(question.id, { options: question.options?.filter((_, i) => i !== optionIndex) })} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-gray-400 hover:bg-red-50 hover:text-red-500"><Trash2 size={16} /></button></div>)}<button onClick={() => patchQuestion(question.id, { options: [...(question.options ?? []), `선택지 ${(question.options?.length ?? 0) + 1}`] })} className="text-xs font-bold text-brand-600">+ 선택지 추가</button></div>}
