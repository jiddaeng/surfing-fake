...
export function LoginPage() {
  ...
  const [email, setEmail] = useState('student@dimigo.hs.kr')
    ...
    <div className="min-h-screen bg-gradient-to-br from-brand-50 via-white to-blue-50 px-4 py-6 sm:py-10">
      <div className="mx-auto max-w-md">
        ...
        <div className="card mt-8 rounded-[1.75rem] p-6 shadow-xl shadow-brand-100/60 sm:p-8">
          ...
          <p className="mt-2 text-sm leading-6 text-gray-500">학교에서 발급받은 시연용 계정으로 로그인하세요.</p>
          ...
          <form onSubmit={submit} className="mt-7 space-y-4">
            <label className="block"><span className="mb-2 block text-sm font-semibold">이메일 또는 로그인 ID</span><div className="relative"><Mail size={17} className="absolute left-3.5 top-3.5 text-gray-400" /><input type="text" required value={email} onChange={(e) => setEmail(e.target.value)} className={`${inputClass} pl-10`} placeholder="이메일 또는 로그인 ID" autoComplete="username" /></div></label>
            <label className="block"><span className="mb-2 block text-sm font-semibold">비밀번호</span><div className="relative"><LockKeyhole size={17} className="absolute left-3.5 top-3.5 text-gray-400" /><input type={showPassword ? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)} className={`${inputClass} px-10`} placeholder="비밀번호 입력" /><button type="button" onClick={() => setShowPassword((show) => !show)} className="absolute right-3 top-3 text-gray-400" aria-label="비밀번호 표시 전환">{showPassword ? <EyeOff size={19} /> : <Eye size={19} />}</button></div></label>
