...
7. 이어서 `supabase/migrations/003_security_hardening.sql`, `004_leader_promotion.sql`, `005_club_catalog.sql`도 순서대로 실행합니다.
8. 네 SQL이 오류 없이 완료되면 성공입니다. 이미 001을 적용한 데이터베이스에도 003~005를 반드시 실행해야 합니다.
...
- 공식 동아리 13개의 카테고리 동기화와 신규 동아리 추가
...
`001_initial_schema.sql` 또는 `003`~`005` migration이 아직 실행되지 않았습니다.
...
현재 계정 역할과 수행하려는 작업이 맞지 않거나 SQL 일부가 실행되지 않은 상태입니다. `profiles`에서 계정 역할을 확인하고 001·003·004·005 SQL 실행 오류가 없었는지 봅니다.
