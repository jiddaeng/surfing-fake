...
6. SQL Editor에 붙여 넣고 `Run`을 누릅니다.
7. 이어서 `supabase/migrations/003_security_hardening.sql`과 `supabase/migrations/004_leader_promotion.sql`도 순서대로 실행합니다.
8. 세 SQL이 오류 없이 완료되면 성공입니다. 이미 001을 적용한 데이터베이스에도 003과 004를 반드시 실행해야 합니다.
...
- 학생의 지원 상태 위조, 교차 동아리 답변, 필수 답변 우회 차단
- 동시 제출·동시 합격 처리의 경쟁 조건 방지
- 입력 크기·형식 제한과 알림 변조 차단
- SVG 업로드 금지와 최소 권한 테이블 권한 적용
- 관리자 전용 동아리장 승격 함수와 실제 변경 결과 검증
...
| 신규 동아리장 | `newleader@dimigo.hs.kr` |
| 관리자 | `admin@dimigo.hs.kr` |

네 계정은 시연용으로 같은 비밀번호를 사용해도 됩니다. 단, Supabase 계정 비밀번호나 Database password를 재사용하지 마세요. 비밀번호는 저장소에 기록하지 않고 면접 전에 별도로 기억해두는 편이 안전합니다.
...
계정 네 개를 만든 다음, 폐기 가능한 로컬/면접 시연 프로젝트에서만 실행합니다. 이 SQL은 기존 지원 데이터를 삭제하므로 운영 프로젝트에서는 실행하면 안 됩니다.

1. `SQL Editor → New query`를 누릅니다.
2. `supabase/demo_seed.sql` 전체를 복사합니다.
3. 붙여 넣고 `Run`을 누릅니다.
4. `시연용 계정 역할과 예시 데이터 입력이 완료되었습니다.`가 나오면 성공입니다.

이 SQL이 만드는 내용:

- 학생, 동아리장, 신규 동아리장, 관리자 역할 설정
- 예시 동아리 9개
- Markdown 상세 소개
- 동아리별 지원 질문
- 학생 계정의 찜 목록
...
예시 동아리 중 `저스트`는 `leader@dimigo.hs.kr` 동아리장 계정과 연결됩니다. `newleader@dimigo.hs.kr` 계정은 아직 담당 동아리가 없도록 남겨두어 새 동아리 등록 흐름을 시연할 수 있습니다. 다른 예시 동아리는 목록 시연용이며 담당자 이름만 들어 있습니다.
...
1. 로그아웃하고 기존 동아리장 계정으로 로그인합니다.
2. 저스트 소개를 Markdown으로 수정합니다.
3. 실시간 미리보기를 확인합니다.
...
1. 로그아웃하고 신규 동아리장 계정으로 로그인합니다.
2. `아직 등록한 동아리가 없어요` 화면에서 동아리 등록으로 이동합니다.
3. 동아리 이름, 카테고리, 한 줄 소개, 상세 Markdown 소개를 작성합니다.
4. 지원 질문을 추가하고 저장합니다.
5. 학생 계정으로 다시 로그인해 새 동아리가 목록에 보이는지 확인합니다.
...
`001_initial_schema.sql`, `003_security_hardening.sql` 또는 `004_leader_promotion.sql`이 아직 실행되지 않았습니다.
...
현재 계정 역할과 수행하려는 작업이 맞지 않거나 SQL 일부가 실행되지 않은 상태입니다. `profiles`에서 계정 역할을 확인하고 001·003·004 SQL 실행 오류가 없었는지 봅니다.

### 동아리장 화면에 동아리가 없음

`supabase/demo_seed.sql`을 계정 생성 전에 실행했을 수 있습니다. 계정 네 개를 만든 뒤 로컬 시연 프로젝트에서 demo seed를 한 번 더 실행하면 저스트와 기존 동아리장 계정이 연결됩니다. 신규 동아리장 계정은 동아리가 없는 상태가 정상입니다.
